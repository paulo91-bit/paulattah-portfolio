...
// Removed the 4 social icons from the import
import { Mail, MapPin, Phone, Server, Shield, Cloud, Terminal, Award, ExternalLink, Code } from 'lucide-react';

/**
 * Paul Attah - The "Daylight Architect" Portfolio
 * Blending Minimalist UI (Inspiration) with Cinematic GSAP Motion (Prompt)
 */
    ...
    <div className="bg-[#FDFDFD] text-[#2D3748] font-sans overflow-x-hidden selection:bg-[#68A893] selection:text-white">
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6'}`}>
        <div className="max-w-7xl mx-auto px-8 flex justify-between items-center">
          <div className="flex gap-6 text-gray-500">
            <a href="https://linkedin.com/in/paul-attah" target="_blank" rel="noreferrer" className="hover:text-[#68A893] transition-colors"><LinkedinIcon size={20} /></a>
          ...
          <a href="mailto:attahattah37@gmail.com" className="flex items-center gap-2 text-sm font-semibold tracking-wider text-gray-500 hover:text-[#68A893] transition-colors uppercase">
            <Mail size={16} /> Get In Touch
          </a>
        </div>
      </nav>

      {/* HERO SECTION (Concentric Inspiration) */}
      <section ref={heroRef} className="relative h-screen flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-40">
          <div className="circle-ring absolute w-[300px] h-[300px] rounded-full border border-gray-200" />
          <div className="circle-ring absolute w-[500px] h-[500px] rounded-full border border-gray-200" />
          <div className="circle-ring absolute w-[700px] h-[700px] rounded-full border border-gray-200" />
          <div className="circle-ring absolute w-[900px] h-[900px] rounded-full border border-gray-200" />
        </div>
        ...
        <div className="relative z-10 flex flex-col items-center hero-element">
          <img 
            ...
            alt="Paul Attah" 
            className="w-32 h-32 rounded-full grayscale hover:grayscale-0 transition-all duration-500 object-cover shadow-lg border-4 border-white mb-8"
          />
          ...
          <div className="flex gap-8 text-xs tracking-widest uppercase text-gray-400 font-semibold mt-4">
            <a href="#about" className="hover:text-[#68A893] transition-colors">About</a>
            <a href="#experience" className="hover:text-[#68A893] transition-colors">Experience</a>
            <a href="#skills" className="hover:text-[#68A893] transition-colors">Skills</a>
            <a href="#certifications" className="hover:text-[#68A893] transition-colors">Certifications</a>
            <a href="#projects" className="hover:text-[#68A893] transition-colors">Projects</a>
          </div>
        </div>
      </section>
      ...
      {/* SKILLS / MICRO-UI SECTION (Adapted from Prompt) */}
      ...
      <section id="projects" ref={projectsRef} className="py-24 px-8 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <ProjectCard 
            ...
            description="AI-powered semantic search engine using PostgreSQL (pgvector) and OpenAI Embeddings to perform high-performance context-aware retrieval." 
            tech={['Python', 'PostgreSQL', 'OpenAI']} 
            link="https://github.com/paulo91-bit/sql-ai-semantic-search" 
          />
          <ProjectCard 
            title="maigie" 
            description="An AI-powered study companion designed to help students organize learning, generate study plans, manage goals, and track progress." 
            tech={['TypeScript', 'React', 'AI']} 
            link="https://github.com/paulo91-bit/maigie" 
          />
          <ProjectCard 
            title="product_backend_devops" 
            description="Infrastructure as Code and CI/CD pipeline demonstrations for automated backend deployments and scaling." 
...
// --- RAW SVG BRAND ICONS (Bulletproof against library updates) ---
  ...
  <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow flex flex-col h-full">
    <div className="flex justify-between items-start mb-6">
      <div className="w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center border border-gray-100">
        <Code size={20} className="text-[#68A893]"/>
      </div>
      <a href={link} target="_blank" rel="noreferrer" className="text-gray-400 hover:text-[#68A893] transition-colors">
    ...
    <div className="flex flex-wrap gap-2 mt-auto">
      {tech.map((t, i) => (
        <span key={i} className="text-[10px] font-bold uppercase tracking-wider text-[#68A893] bg-[#68A893]/10 px-3 py-1 rounded-full">
          {t}
        </span>
      ))}
    </div>
...
const ExperienceCard = ({ title, company, date, icons, bullets }) => (
  <div className="min-w-[320px] md:min-w-[400px] snap-center bg-white p-8 rounded-3xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
    ...
    <ul className="space-y-3 text-sm text-gray-500">
      {bullets.map((b, i) => (
        <li key={i} className="flex gap-3 items-start">
          <span className="text-[#68A893] mt-1">•</span>
...
// Light Mode Adaptations of the "Organic Tech" prompt features
  ...
  return (
    <div className="relative h-[300px] bg-white rounded-[2rem] p-8 overflow-hidden border border-gray-100 shadow-sm flex flex-col justify-between">
      <div className="flex justify-between items-start text-gray-400">
        <Cloud size={20} />
        <span className="text-[10px] tracking-widest uppercase font-bold">Skill Shuffler</span>
      </div>
      <div className="relative flex-1 flex flex-col items-center justify-center mt-8">
        {labels.map((label, i) => (
          <div 
            key={label}
            className="absolute bg-gray-50 border border-gray-100 rounded-2xl p-6 w-full max-w-[240px] shadow-sm transition-all duration-[800ms] ease-[cubic-bezier(0.34,1.56,0.64,1)]"
            style={{
              transform: `translateY(${(i - 1) * 40}px) scale(${1 - i * 0.1})`,
              zIndex: 10 - i,
              opacity: 1 - i * 0.3
            }}
          >
    ...
    <div className="h-[300px] bg-[#1A202C] rounded-[2rem] p-8 shadow-sm flex flex-col justify-between relative overflow-hidden">
      <div className="flex justify-between items-start text-gray-500">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] uppercase font-bold tracking-widest">Live Terminal</span>
        </div>
      </div>
      <div className="font-mono text-green-400 text-sm mt-8">
        <span className="text-gray-500 mr-2">paulo@root:~$</span>
        {text}<span className="inline-block w-2 h-4 bg-green-400 ml-1 animate-pulse" />
      </div>
      <div className="space-y-1 font-mono text-[10px] text-gray-600 mt-auto">
        <div>[OK] INIT_INFRASTRUCTURE</div>
        <div>[OK] SECRETS_LOADED</div>
        <div>[PENDING] AWAITING_COMMAND</div>
      </div>
    </div>
  );
};
